            ...
            if (data.length === 0) {
                ...
                data.forEach((item) => {
                    ...
                    <tr>
                        ...
                        <td>
                            ${item.category === 'vendor_outcome' ? '<span class="badge rounded-pill text-danger-emphasis bg-danger-subtle border border-danger-subtle">Cash out</span>' : '<span class="badge rounded-pill text-success-emphasis bg-success-subtle border border-success-subtle">Penjualan</span>'}
                        </td>
        ...
        if (data.length === 0) {
            ...
            data.forEach((item) => {
                ...
                <tr>
                    <td class="text-center">${number}</td>
                    <td>Rp${formatRupiah(item.credit)}</td>
                    <td>${item.category === 'vendor_outcome' ? '<span class="badge rounded-pill text-danger-emphasis bg-danger-subtle border border-danger-subtle">Cash out</span>' : '<span class="badge rounded-pill text-success-emphasis bg-success-subtle border border-success-subtle">Penjualan</span>'}
                    </td>
                    <td>${moment(item.created_at).format('dddd, D MMMM YYYY H:mm:ss')}</td>
                </tr>`;
                number++;
